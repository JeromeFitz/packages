...
const items: IItems[] = [
  { title: "Default", toast: { description } },
  {
    title: "Multi-Line",
    toast: {
      description: [description, description, description, description].join(descriptionJoiner),
    },
  },
  {
    title: "Action",
...
const ToastDemo = () => {
  ...
  const debouncedDescriptionHandler = useMemo(() => _debounce(handleDescriptionChange, 300), []);
  useEffect(() => {
    return () => {
    ...
    <>
      ...
      <>
        ...
        <Flex css={{ flexDirection: "column", gap: "$4" }}>
          ...
          <Flex css={{ "@bp1": { maxWidth: "50%" }, alignItems: "center", mb: "$2" }}>
            <Button
              onClick={() => {
                if (toaster?.current) {
