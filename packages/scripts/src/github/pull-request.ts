...
async function setPullRequest({ base, dryRun = false, head, labels, owner, q, repo, version }) {
  try {
    ...
    if (doesGitflowPrExists) {
      ...
      console.log(colorize`{orange.bold 🤔️  ${pull_number} => Exists, should we update PR ?}`);
      console.log(colorize`{orange.bold 🤪️  Right now we are not}`);
      console.log(colorize`{orange.bold 😵️  https://github.com/${repo_id}/pull/${pull_number}}`);
