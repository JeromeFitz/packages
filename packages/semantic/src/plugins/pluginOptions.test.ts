...
describe("getPluginOptions", () => {
  ...
  describe("default enabled set", () => {
    ...
    it("commit-analyzer is always first", () => {
      expect(pluginName(getPluginOptions()[0])).toBe("@semantic-release/commit-analyzer");
    });
  });
  ...
  describe("enableGit", () => {
    ...
    it("true includes git plugin", () => {
      expect(pluginNames(getPluginOptions({ enableGit: true }))).toContain("@semantic-release/git");
    });
  });
  ...
  describe("enableReleaseNotesCustom", () => {
    it("false excludes custom release-notes-generator", () => {
      expect(pluginNames(getPluginOptions({ enableReleaseNotesCustom: false }))).not.toContain(
  ...
  describe("pkgRoot", () => {
    it("defaults to ./dist", () => {
      ...
      const npmPlugin = plugins.find((p) => pluginName(p) === "@semantic-release/npm") as [
        string,
        { pkgRoot: string },
      ];
