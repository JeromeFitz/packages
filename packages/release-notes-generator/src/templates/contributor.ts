...
const contributor = async (
  ...
  const prohibitLogins = [...contributorsProhibitListDefault.login, ...prohibitList.login];
  for (const ejectLogin of prohibitLogins) {
    const idx = authors.findIndex((a) => a.login === ejectLogin);
    if (idx !== -1) authors.splice(idx, 1);
  }

  const prohibitEmails = [...contributorsProhibitListDefault.email, ...prohibitList.email];
