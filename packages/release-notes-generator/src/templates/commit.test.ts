...
import type { CommitGroup, MarkdownContext, RenderMeta, TransformedCommit } from "../types";
...
describe("commit template", () => {
  ...
  it("matches snapshot", () => {
    ...
    expect(commit(makeContext([c], { linkReferences: true }), [c], baseMeta)).toMatchSnapshot();
  });
});
