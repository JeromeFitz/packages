...
const Callout = (props: CalloutProps) => {
  ...
  return (
    <StyledCallout variant={variant} {...rest}>
      {icon ? <StyledCalloutIconWrapper variant={variant}>{icon}</StyledCalloutIconWrapper> : null}
      {label ? (
        <StyledCalloutLabelWrapper variant={variant}>{label}</StyledCalloutLabelWrapper>
      ) : null}
      {children}
    </StyledCallout>
  );
