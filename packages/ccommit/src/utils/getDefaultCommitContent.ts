...
import { COMMIT_MODES } from '~ccommit/lib'
import { getIssueTracker } from '~ccommit/utils'
...
const getDefaultCommitContent = (options) => {
  /*
    Since the hook is called with `ccommit --hook $1`
  ...
  */
  ...
  if (options.mode === COMMIT_MODES.CLIENT || !fs.existsSync(commitFilePath)) {
    return {
      ...
      title: options['title'] || getIssueTracker() || null,
    }
  }
