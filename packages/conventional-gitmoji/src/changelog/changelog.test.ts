...
describe('changelog: transformer', () => {
  ...
  it('emoji-typed commit', () => {
    expect(
      transformer(make({ subject: '✨ add new feature', type: '✨' }), {}),
    ).toMatchSnapshot()
  })

  it('word-typed commit', () => {
    expect(
      transformer(make({ subject: 'resolve the issue', type: 'fix' }), {}),
    ).toMatchSnapshot()
  })

  it('unknown type returns undefined', () => {
    expect(
      transformer(make({ subject: 'no match', type: 'zzz-nope' }), {}),
    ).toBeUndefined()
  })
})
...
describe('changelog: recommendedBump.whatBump', () => {
  it('patch when no feat / breaking', () => {
    expect(
      recommendedBumpOpts.whatBump([{ notes: [], type: 'fix' }]),
    ).toMatchSnapshot()
  })

  it('minor on feat (word)', () => {
    expect(
      recommendedBumpOpts.whatBump([{ notes: [], type: 'feat' }]),
    ).toMatchSnapshot()
  })
